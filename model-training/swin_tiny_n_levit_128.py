...
# ============================================================
# CELL 10 — PER-CLASS SHAP FORCE PLOTS (Most Confidently Predicted Sample)
# ============================================================
print("\n--- Generating Per-Class Force Plots ---")
force_start = time.time()

# For each class, find the correctly-predicted test sample the model
# was MOST confident about (highest predicted probability for that class).
indices_to_plot = []
for target_cls in range(num_classes):
    candidates = [i for i in range(len(y_test))
                  if y_test[i] == target_cls and y_pred[i] == target_cls]
    if not candidates:
        print(f"WARNING: no correctly predicted sample for class {class_names[target_cls]}")
        continue
    best_i = max(candidates, key=lambda i: y_prob[i, target_cls])
    indices_to_plot.append((target_cls, best_i))

for cls_idx, test_idx in indices_to_plot:
    plt.figure(figsize=(10, 3))
    plt.title(f"SHAP Force Plot — {class_names[cls_idx]} "
              f"(confidence={y_prob[test_idx, cls_idx]:.3f})", fontsize=12)
    shap.plots._waterfall.waterfall_legacy(
        explainer_tree.expected_value[cls_idx],
        shap_list[cls_idx][test_idx]
    )
    plt.tight_layout()
    plt.savefig(f"force_plot_{class_names[cls_idx].replace(' ', '_')}.png",
                dpi=150, bbox_inches='tight')
    plt.show()

print(f"Force plot generation time: {time.time() - force_start:.2f}s")
